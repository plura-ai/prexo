...
const requestCounts = new Map<string, Map<string, { count: number; timestamp: number }>>();
...
export const computeCost = (
  ...
  console.log("\x1b[36m[computeCost] Existing entry for tag:\x1b[0m", tag, entry);

  if (!entry || now - entry.timestamp > timeF) {
    console.log("\x1b[32m[computeCost] Creating new entry or resetting expired entry for tag\x1b[0m", tag);
    tagMap.set(tag, { count: 1, timestamp: now });
    console.log("\x1b[32m[computeCost] Returning 0 cost for new/reset entry (tag)\x1b[0m", tag);
    return 0;
  }
